...
for col, (label, (f_precio, f_costo, detalle)) in zip(cols, escenarios.items()):
    ...
    with col:
        ...
        st.metric("Capital de trabajo", format_currency(metrics['MaxFinancingNeed']))

# 5. Distribuciones (Monte Carlo)
if df_mc is not None:
    st.markdown("### 6. Distribuciones de resultados (Monte Carlo)")
    st.caption("Cómo leer este bloque: cada distribución resume la frecuencia de resultados posibles para TIR, ventas y costos bajo incertidumbre.")
    c1, c2 = st.columns(2)
    with c1:
